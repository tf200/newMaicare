<script lang="ts">
	...
	import { Loader2, Save, Sparkles, Target, Check, AlertCircle } from 'lucide-svelte';
	...
	interface Props {
		open: boolean;
		clientId: string;
		onSave: (data: CreateGoalRequest) => Promise<void> | void;
		onCancel: () => void;
		onGenerate?: (topicId: string) => Promise<GenerateGoalsResponse>;
	}

	let { open = $bindable(false), clientId, onSave, onCancel, onGenerate }: Props = $props();
	...
	const generateGoal = async () => {
		...
		try {
			if (onGenerate) {
				const res = await onGenerate(selectedTopicId);
				...
				const topicName = topics.find((t) => t.id === selectedTopicId)?.topic_name ?? '';
				title = m.goal_suggestion_title({ name: topicName });
...
<Modal bind:open title={m.create_goal()} description={m.create_goal_description()} size="lg">
	{#snippet footer()}
				...
				>
					<div class="flex items-center justify-between">
						...
						<button
							...
							class="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm transition-all hover:from-purple-500 hover:to-indigo-500 disabled:opacity-50"
						>
							{#if isGenerating}

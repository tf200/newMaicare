		...
		documents = documents.map((d) =>
			d.id === doc.id
				? {
						...
						signature_hash: 'sha256-' + Math.random().toString(36).slice(2, 14)
					}
				: d
		);
		signModal = null;
	...
	<div class="flex flex-col gap-3 sm:flex-row">
		<div class="relative flex-1">
			...
			<input
				...
				class="h-9 w-full rounded-xl border border-border bg-surface px-3 pl-9 text-sm text-text transition-colors placeholder:text-text-subtle focus:border-sky-500 focus:ring-1 focus:ring-sky-500/20 focus:outline-none"
			/>
		</div>
		<select
			class="h-9 rounded-xl border border-border bg-surface px-3 text-xs font-semibold text-text focus:border-sky-500 focus:ring-1 focus:ring-sky-500/20 focus:outline-none"
			value={statusFilter}
	...
	{:else}
		<div class="space-y-2">
			{#each filtered as doc, idx (doc.id)}
				...
				<div
					class="group flex items-center justify-between gap-4 rounded-xl border border-border/50 bg-surface px-4 py-3 transition-all hover:border-border hover:shadow-sm {doc.status ===
					'rejected'
					...
					<div class="flex min-w-0 flex-1 items-center gap-3">
						...
						<div class="min-w-0">
							<div class="flex items-center gap-2">
								<span class="truncate text-[13px] font-bold text-text">{doc.document_title}</span>
								<span
					...
					<div class="flex shrink-0 items-center gap-4">
						...
						{#if doc.signed_at}
							<div class="hidden text-right md:block">
								...
								<div class="text-[12px] text-emerald-600 tabular-nums dark:text-emerald-400">
...
{#if signModal}
	...
	>
		<div class="space-y-5">
			<div class="space-y-2 rounded-xl border border-border bg-border/20 p-4">
				...
				<div class="flex items-center gap-4 text-xs text-text-muted">
					...
					<span
						>{m.ondertekenen_type()}: {docTypeConfig[signModal.document_type]?.label ??
							signModal.document_type}</span
					>
				</div>
			</div>

			<label
				class="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4 transition-colors hover:bg-border/10 {agreedToTerms
			...
			<div class="flex justify-end gap-2">
				...
				<button
					...
					class="flex h-9 items-center gap-1.5 rounded-xl bg-emerald-600 px-4 text-sm font-semibold text-white transition-all hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40"
				>
					<PenLine class="h-3.5 w-3.5" />
		...
		<div class="space-y-5">
			...
			<div class="space-y-1.5">
				...
				<textarea
					...
					class="w-full resize-none rounded-xl border border-border bg-surface px-3 py-2 text-sm text-text transition-colors placeholder:text-text-subtle focus:border-rose-500 focus:ring-1 focus:ring-rose-500/20 focus:outline-none"
				></textarea>
			</div>
			...
			<div class="flex justify-end gap-2">
				...
				<button
					...
					class="flex h-9 items-center gap-1.5 rounded-xl bg-rose-600 px-4 text-sm font-semibold text-white transition-all hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-40"
				>
					<XCircle class="h-3.5 w-3.5" />
...
<!-- View Modal -->
{#if viewModal}
	<Modal open={true} title={m.ondertekenen_document()} size="md">
		{@const cfg = statusConfig[viewModal.status]}
		<div class="space-y-4">
			<div class="space-y-3">
				<div class="flex items-center justify-between">
					<span class="text-xs font-bold tracking-wider text-text-subtle uppercase"
						>{m.ondertekenen_document()}</span
					>
					<span
						class="inline-flex items-center gap-1 rounded-lg {cfg.color} px-2 py-0.5 text-[11px] font-bold"
					>
			...
			<div class="grid grid-cols-2 gap-3 rounded-xl border border-border bg-border/10 p-4">
				<div>
					<div class="text-[10px] font-bold text-text-subtle uppercase">
						{m.ondertekenen_employee()}
					</div>
				...
				{#if viewModal.signed_at}
					<div>
						...
						<div class="text-sm font-semibold text-emerald-600 dark:text-emerald-400">
							{formatDate(viewModal.signed_at)}
						</div>
					</div>
				{/if}
			...
			{#if viewModal.rejection_reason}
				<div
					class="rounded-xl border border-rose-200 bg-rose-50/50 p-3 dark:border-rose-800/30 dark:bg-rose-950/20"
				>
					<div class="mb-1 text-[10px] font-bold text-rose-500 uppercase">
						{m.ondertekenen_reject_reason()}
					</div>
			...
			{#if viewModal.signature_hash}
				<div class="rounded-xl border border-border bg-border/10 p-3">
					<div class="mb-1 text-[10px] font-bold text-text-subtle uppercase">Hash</div>

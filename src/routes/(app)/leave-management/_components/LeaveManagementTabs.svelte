...
<div class="w-full rounded-[1.75rem] border border-zinc-200 bg-white p-2.5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-3">
	<div class="flex flex-nowrap items-center gap-1 overflow-x-auto scrollbar-hide">
		{#each actionTabs as tab}
			...
			>
				<tab.icon class="h-4 w-4 {activeTab === tab.id ? 'text-teal-600 dark:text-teal-400' : ''}" />
				<span class="whitespace-nowrap">{tab.label}</span>
				{#if activeTab === tab.id}
					<div class="absolute -bottom-1 left-1/2 h-1 w-8 -translate-x-1/2 rounded-full bg-teal-500/40 blur-sm"></div>
				{/if}
			</button>
		{/each}
		...
		{#each viewTabs as tab}
			...
			>
				...
				{#if tab.id === 'overview' && pendingCount > 0}
					<span class="flex h-5 min-w-5 items-center justify-center rounded-full bg-orange-500 px-1.5 text-[10px] font-black text-white shadow-lg shadow-orange-500/30">
						{pendingCount}
					</span>
				{/if}

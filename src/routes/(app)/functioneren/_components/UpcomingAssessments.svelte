<script lang="ts">
	import { AlertTriangle, Clock, CalendarClock, Mail, ChevronRight, ArrowRight } from 'lucide-svelte';
	...
	const fmtDateFull = (d: string) =>
		new Intl.DateTimeFormat('nl-NL', { weekday: 'short', day: 'numeric', month: 'long' }).format(new Date(d));
...
<div>
	<div class="mb-5 flex items-baseline justify-between">
		<div>
			...
			<p class="mt-0.5 text-xs font-medium text-text-subtle">6-wekelijkse cyclus na laatste beoordeling</p>
		</div>
		{#if overdueCount > 0 || dueSoonCount > 0}
			<div class="flex gap-2">
				{#if overdueCount > 0}
					<span class="inline-flex items-center rounded-xl bg-error/10 px-2.5 py-1 text-[10px] font-bold text-error">
						<AlertTriangle class="mr-1 h-3 w-3" /> {overdueCount} verlopen
					</span>
				{/if}
				{#if dueSoonCount > 0}
					<span class="inline-flex items-center rounded-xl bg-amber-500/10 px-2.5 py-1 text-[10px] font-bold text-amber-600 dark:text-amber-400">
						<Clock class="mr-1 h-3 w-3" /> {dueSoonCount} binnenkort
					</span>
				{/if}
			</div>
		{/if}
	...
	{#if loading}
		<div class="grid gap-3 sm:grid-cols-2">
			{#each Array(4) as _, i}
				<div class="h-[120px] animate-pulse rounded-2xl bg-border/20" style="animation-delay: {i * 60}ms"></div>
			{/each}
		</div>
			...
			{#if overdueCount > 0}
				<div>
					<div class="mb-3 flex items-center gap-2">
						...
						<span class="text-[10px] font-bold tracking-[0.14em] text-error uppercase">Verlopen</span>
					...
					<div class="grid gap-3 sm:grid-cols-2">
						{#each items.filter((i) => i.isOverdue) as item, idx (item.employeeId)}
							...
							<div
								...
								style="animation: fade-in 350ms cubic-bezier(0.22,1,0.36,1) both; animation-delay: {idx * 60}ms"
							>
								<!-- Progress bar at top -->
								...
								<div class="p-5">
									<div class="flex items-start justify-between">
										<div class="flex items-center gap-3">
											<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-error/10 text-[12px] font-bold tracking-tight text-error">
												{item.employeeName.split(' ').map((p) => p[0]).join('').slice(0, 2)}
											</div>
											<div>
										...
										<div class="text-right">
											<div class="text-[24px] font-extrabold leading-none tabular-nums text-error">{Math.abs(days)}</div>
											<div class="text-[10px] font-bold text-error/70">{Math.abs(days) === 1 ? 'dag' : 'dagen'} te laat</div>
										</div>
									</div>
									...
									<div class="mt-4 flex items-center justify-between">
										...
										<button class="flex h-8 items-center gap-1.5 rounded-lg bg-error/10 px-3 text-[11px] font-bold text-error transition-all hover:bg-error/15 active:scale-95">
			...
			{#if dueSoonCount > 0}
				<div>
					<div class="mb-3 flex items-center gap-2">
						...
						<span class="text-[10px] font-bold tracking-[0.14em] text-amber-600 dark:text-amber-400 uppercase">Binnen 2 weken</span>
						...
						{#each items.filter((i) => i.isDueSoon) as item, idx (item.employeeId)}
							...
							>
								...
								<div class="p-5">
									<div class="flex items-start justify-between">
										<div class="flex items-center gap-3">
											<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-500/10 text-[12px] font-bold tracking-tight text-amber-600 dark:text-amber-400">
												{item.employeeName.split(' ').map((p) => p[0]).join('').slice(0, 2)}
											</div>
											<div>
												<div class="text-[14px] font-bold text-text">{item.employeeName}</div>
										...
										<div class="text-right">
											<div class="text-[24px] font-extrabold leading-none tabular-nums text-amber-600 dark:text-amber-400">{days}</div>
											<div class="text-[10px] font-bold text-amber-600/60 dark:text-amber-400/60">{days === 1 ? 'dag' : 'dagen'}</div>
										</div>
									</div>
									...
									<div class="mt-4 flex items-center justify-between">
										...
										<button class="flex h-8 items-center gap-1.5 rounded-lg border border-amber-200 px-3 text-[11px] font-bold text-amber-600 transition-all hover:bg-amber-500/5 active:scale-95 dark:border-amber-800 dark:text-amber-400">
											<Mail class="h-3 w-3" /> Uitnodigen
										</button>
									</div>
								</div>
			...
			{#if plannedCount > 0}
				<div>
					<div class="mb-3 flex items-center gap-2">
						...
						<span class="text-[10px] font-bold tracking-[0.14em] text-text-subtle uppercase">Gepland</span>
						...
						{#each items.filter((i) => !i.isOverdue && !i.isDueSoon) as item, idx (item.employeeId)}
							...
							>
								...
								<div class="h-1 bg-border/30">
									<div class="h-full bg-text-subtle/30 transition-all" style="width: {progress}%"></div>
								</div>

								<div class="p-4">
									<div class="flex items-center gap-3">
										<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-border/40 text-[11px] font-bold tracking-tight text-text-muted">
											{item.employeeName.split(' ').map((p) => p[0]).join('').slice(0, 2)}
										</div>
										<div class="min-w-0 flex-1">
											<div class="truncate text-[13px] font-bold text-text">{item.employeeName}</div>
											<div class="mt-0.5 text-[11px] text-text-subtle">{fmtDateFull(item.nextAssessmentDate)}</div>
										</div>
										<div class="text-right">
											<div class="text-[18px] font-extrabold leading-none tabular-nums text-text-muted">{days}</div>

<script lang="ts">
	...
	const doneCount = $derived(
		assignments.filter((a) => ['approved', 'revision_needed'].includes(a.status)).length
	);
	...
	<div>
		<div class="mb-5 flex items-baseline justify-between">
			...
			<span class="text-xs font-semibold text-text-subtle tabular-nums"
				>{m.func_advice_count({ count: assignments.length })}</span
			>
		</div>
		...
		{#if loading}
			<div class="space-y-2 py-4">
				{#each Array(4) as _, i}
					<div
						class="h-[52px] animate-pulse rounded-xl bg-border/20"
				...
				<table class="w-full">
					<thead>
						<tr class="border-b border-border">
							<th
								class="pt-0 pb-2.5 text-left text-[10px] font-bold tracking-[0.14em] text-text-subtle uppercase"
								>{m.salaris_employee()}</th
							>
							<th
								class="pt-0 pb-2.5 text-left text-[10px] font-bold tracking-[0.14em] text-text-subtle uppercase"
								>{m.func_domain()}</th
							...
							>
							<th
								class="pt-0 pb-2.5 text-center text-[10px] font-bold tracking-[0.14em] text-text-subtle uppercase"
								>{m.func_status_label()}</th
							>
							<th class="w-24 pt-0 pb-2.5"></th>
						</tr>
					</thead>
					<tbody>
						{#each assignments as a, idx (a.id)}
							<tr
								class="group border-b border-border/50 transition-colors hover:bg-rose-500/[0.03] {idx %
									2 ===
								0
									? ''
									: 'bg-border/[0.06]'}"
							>
								<td class="py-3 text-[13px] font-bold text-text"
									>{a.employee?.name ?? 'Onbekend'}</td
								>
								<td class="py-3">
									<span
										class="inline-flex items-center rounded-lg border border-border/60 bg-border/20 px-2 py-0.5 text-[10px] font-bold text-text-muted"
									>
										{domainLabels[a.domain_id] ?? a.domain_id}
									</span>
								</td>
								<td class="max-w-[300px] truncate py-3 text-[13px] text-text">{a.question_text}</td>
								<td class="py-3 text-[13px] text-text-muted tabular-nums">{fmtDate(a.due_date)}</td>
								<td class="py-3 text-center">
									{#if a.status === 'open'}
										<span
											class="inline-flex items-center rounded-lg bg-amber-500/10 px-2 py-1 text-[10px] font-bold text-amber-600 dark:text-amber-400"
										>
									...
									{:else if a.status === 'submitted'}
										<span
											class="inline-flex items-center rounded-lg bg-indigo-500/10 px-2 py-1 text-[10px] font-bold text-indigo-600 dark:text-indigo-400"
										>
											<Send class="mr-1 h-2.5 w-2.5" />
											{m.func_status_submitted()}
										</span>
									{:else if a.status === 'approved'}
										<span
											class="inline-flex items-center rounded-lg bg-emerald-500/10 px-2 py-1 text-[10px] font-bold text-emerald-600 dark:text-emerald-400"
										>
											<CheckCircle class="mr-1 h-2.5 w-2.5" />
											{m.func_status_approved()}
										</span>
									{:else}
										<span
											class="inline-flex items-center rounded-lg bg-error/10 px-2 py-1 text-[10px] font-bold text-error"
										>
											<AlertCircle class="mr-1 h-2.5 w-2.5" />
											{m.func_status_revision()}
										</span>
									{/if}
								</td>
								<td class="py-3">

<script lang="ts">
	...
	function ratingVisual(r: number): { label: string; cls: string; bar: string } {
		...
		if (r >= 6)
			return { label: 'Voldoende', cls: 'text-teal-700 dark:text-teal-300', bar: 'bg-teal-500' };
		if (r === 5)
			return { label: 'Matig', cls: 'text-amber-700 dark:text-amber-300', bar: 'bg-amber-500' };
...
{#if assessment}
	<div class="space-y-6">
		...
		<div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
			{#each [{ label: 'Medewerker', value: assessment.employee?.name ?? '—' }, { label: 'Datum', value: new Intl.DateTimeFormat( 'nl-NL', { day: 'numeric', month: 'long', year: 'numeric' } ).format(new Date(assessment.assessment_date)) }, { label: 'Score', value: assessment.total_score !== null ? `${assessment.total_score.toFixed(1)}/10` : '—' }, { label: 'Status', value: assessment.status === 'completed' ? 'Voltooid' : 'Concept' }] as item}
				<div class="rounded-xl border border-border/60 bg-surface px-4 py-3">
					<div class="text-[10px] font-bold tracking-[0.14em] text-text-subtle uppercase">
						{item.label}
					</div>
		...
		{#if assessment.notes}
			<div
				class="rounded-xl border border-rose-200/50 bg-rose-50/50 p-4 dark:border-rose-800/30 dark:bg-rose-950/20"
			>
				<div
					class="text-[10px] font-bold tracking-[0.14em] text-rose-600/70 uppercase dark:text-rose-400/70"
				>
					Opmerkingen
				</div>
				<p class="mt-1 text-[13px] leading-relaxed text-rose-900 dark:text-rose-100">
					{assessment.notes}
				</p>
			</div>
		{/if}
				...
				<div>
					<div class="mb-3 flex items-baseline justify-between border-b border-border pb-2">
						<div class="flex items-center gap-2">
							<span
								class="inline-flex h-6 w-8 items-center justify-center rounded-md bg-rose-500/8 text-[10px] font-bold text-rose-600 dark:text-rose-400"
							>
								{domainShort[domainId] ?? '?'}
							</span>
							<h3 class="text-[13px] font-extrabold text-text">
						...
						<div class="flex items-center gap-2">
							<div class="h-1.5 w-10 overflow-hidden rounded-full bg-border/50">
								<div
									class="h-full rounded-full {avgVis.bar}"
					...
					<div class="space-y-1.5">
						{#each domainScores as score}
							...
							<div
								class="flex items-start gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-border/20"
							>
								<div class="min-w-0 flex-1">
									<div class="text-[13px] font-semibold text-text">
										{itemLabels[score.item_id] ?? score.item_id}
									</div>
									{#if score.remarks}
										<div
											class="mt-1 flex items-start gap-1.5 text-[11px] leading-snug text-text-muted"
										>
								...
								<div class="flex shrink-0 items-center gap-2">
									<div class="h-1.5 w-8 overflow-hidden rounded-full bg-border/50">
										<div
											class="h-full rounded-full {rv.bar}"
											style="width: {(score.rating / 10) * 100}%"
										></div>
									</div>
									<span class="w-6 text-right text-xs font-extrabold tabular-nums {rv.cls}"
										>{score.rating}</span
									>
								</div>
							</div>

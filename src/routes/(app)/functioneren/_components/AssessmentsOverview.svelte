...
<div>
	<div class="mb-5 flex items-baseline justify-between">
		...
		<span class="text-xs font-semibold text-text-subtle tabular-nums"
			>{assessments.length} totaal</span
		>
	</div>
	...
	{:else if assessments.length === 0}
		<div class="flex flex-col items-center rounded-2xl border border-dashed border-border py-16">
			<div
				class="flex h-14 w-14 items-center justify-center rounded-2xl bg-rose-500/8 dark:bg-rose-500/5"
			>
						...
						>
							<td class="py-3">
								<div class="flex items-center gap-3">
									<div
										class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-rose-500/8 text-[11px] font-bold tracking-tight text-rose-600 dark:text-rose-400"
									>
										{a.employee?.name
											?.split(' ')
											.map((p) => p[0])
									...
									<div>
										<div class="text-[13px] font-bold text-text">
											{a.employee?.name ?? 'Onbekend'}
										</div>
									</div>
								</div>
							</td>
							<td class="py-3 text-[13px] text-text-muted tabular-nums"
								>{fmtDate(a.assessment_date)}</td
							>
							<td class="py-3">
								<div class="flex items-center gap-2.5">
									<div class="h-1.5 w-16 overflow-hidden rounded-full bg-border/50">
										<div
											class="h-full rounded-full {si.bar} transition-all"
											style="width: {a.total_score !== null ? (a.total_score / 10) * 100 : 0}%"
										></div>
									</div>
							...
							<td class="py-3 text-center">
								{#if a.status === 'completed'}
									<span
										class="inline-flex items-center rounded-lg bg-emerald-500/10 px-2.5 py-1 text-[10px] font-bold tracking-wide text-emerald-700 dark:text-emerald-400"
										>{m.func_status_completed()}</span
									>
								{:else}
									<span
										class="inline-flex items-center rounded-lg bg-border/40 px-2.5 py-1 text-[10px] font-bold tracking-wide text-text-muted"
										>{m.func_status_draft()}</span
									>
								{/if}
							</td>
							<td class="py-3">

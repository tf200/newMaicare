...
export const mapInCareClient = (payload: GetClientResponse): ClientOverviewData => {
	...
	return {
		...
		registrationStatus: payload.status === 'in_care' ? 'In care' : payload.status,
		...
		intakeSummary: payload.intake
			? {
					...
					participants: [],
					evaluationIntervalWeeks: payload.care?.evaluation_intervals_weeks ?? 0,
		...
		contractSummary: payload.contract_summary
			? {

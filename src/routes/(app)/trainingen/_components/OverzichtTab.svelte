...
<div>
	...
	<div class="mb-5 flex flex-col gap-3 sm:flex-row">
		...
		<select
			class="h-9 rounded-xl border border-border bg-surface px-3 text-xs font-semibold text-text focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/20"
			value={statusFilter}
	...
	{#if loading}
		<div class="space-y-2 py-4">
			{#each Array(6) as _, i}
				<div class="h-[56px] animate-pulse rounded-xl bg-border/20" style="animation-delay: {i * 50}ms"></div>
			{/each}
		</div>
	...
	{:else}
		<div class="space-y-2">
			{#each filtered as et, idx (et.id)}
				...
				<div
					class="group flex items-center justify-between gap-4 rounded-xl border border-border/50 bg-surface px-4 py-3 transition-all hover:border-border hover:shadow-sm {et.status === 'expired' ? '!border-rose-200/60 dark:!border-rose-800/30' : ''}"
					...
					<div class="flex items-center gap-3 min-w-0 flex-1">
						...
						<div class="min-w-0">
							<div class="flex items-center gap-2">
								<span class="text-[13px] font-bold text-text truncate">{et.training_name}</span>
								{#if et.status === 'planned' || et.status === 'in_progress'}
									<span class="inline-flex items-center rounded-md {cfg.color} px-1.5 py-0.5 text-[9px] font-bold">{cfg.label}</span>
								{/if}
							</div>

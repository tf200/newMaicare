...
<div>
	...
	{#if loading}
		<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
			{#each Array(6) as _, i}
				<div class="h-[140px] animate-pulse rounded-2xl bg-border/20" style="animation-delay: {i * 50}ms"></div>
			{/each}
		</div>
	{:else}
		<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
			{#each trainings as training, idx (training.id)}
				...
				<div
					...
					style="animation: fade-in 350ms cubic-bezier(0.22,1,0.36,1) both; animation-delay: {idx * 50}ms"
				>
					...
					{#if training.description}
						<p class="mt-1 text-[12px] leading-relaxed text-text-subtle line-clamp-2">{training.description}</p>
					{/if}

					<div class="mt-3 flex flex-wrap gap-1.5">
						<span class="inline-flex items-center rounded-md {cat.bg} px-2 py-0.5 text-[10px] font-bold {cat.text} capitalize">{training.category}</span>
						{#if training.duration_hours}
							<span class="inline-flex items-center rounded-md bg-border/30 px-2 py-0.5 text-[10px] font-bold text-text-muted">{training.duration_hours}u</span>
						{/if}
						{#if training.validity_months}
							<span class="inline-flex items-center rounded-md bg-border/30 px-2 py-0.5 text-[10px] font-bold text-text-muted">{training.validity_months}m geldig</span>
						{/if}
					</div>
				</div>
			{/each}
